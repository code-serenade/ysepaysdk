...
type T1SmscSignParam struct {
	...
	CodeScanD0Fee string `json:"codeScanD0Fee,omitempty"` // 扫码实时到账垫资费,busOpenType=01时必填
	// TODO 这里只添加了扫码实时到账参数
}

type CodeScanD0FeeParam struct {
	RateType   string `json:"rateType,omitempty"`   // 收费方式,只支持0 按百分比
	RateFee    string `json:"rateFee,omitempty"`    // 费率,单位为% 填0.53代表0.53%
	RateBottom string `json:"rateBottom,omitempty"` // 最低收费,单位为：分
}
